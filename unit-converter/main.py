...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.geometry('500x200+0+0') # defines window dimensions
        self.resizable(0, 0) # disables window resizing

        # define text fonts
        self.normat_text = font.Font(family="Helvetica", size=10)
        self.bold_text = font.Font(family="Helvetica", size=10, weight='bold')
    ...
    def create_containers(self):
        ...
        self.header_frame = tk.Frame(self, width=500, height=50)
        self.input_frame = tk.Frame(self, width=250, height=150)
        self.output_frame = tk.Frame(self, width=250, height=150)

        # place frames on window grid
        self.header_frame.grid(row=0, column=0, columnspan=2, padx=5, pady=5, sticky='nsew')
        self.input_frame.grid(row=1, column=0, padx=5, pady=5, sticky='nsew')
        self.output_frame.grid(row=1, column=1, padx=5, pady=5, sticky='nsew')
    ...
    def create_input_frame_widgets(self):
        ...
        self.input_entry = tk.Entry(self.input_frame, textvariable=self.input_value, font=self.normat_text)
        
        # create input unit dropdown
        self.input_unit=tk.StringVar()
        self.input_unit_cbbx = ttk.Combobox(self.input_frame, values=self.units, textvariable=self.input_unit, width=10, state='readonly')

        # create reset fields button
        self.reset_btn = tk.Button(self.input_frame, text='Clear', command=self.reset_fields)
        ...
        self.reset_btn.grid(row=3, column=0, padx=5, pady=5, sticky='w')
    ...
    def create_output_frame_widgets(self):
        ...
        self.output_entry = tk.Entry(self.output_frame, textvariable=self.output_value, font=self.normat_text, state='readonly')

        # create output unit dropdown
        self.output_unit=tk.StringVar()
        self.output_unit_cbbx = ttk.Combobox(self.output_frame, values=self.units, textvariable=self.output_unit, width=10, state='readonly')

        # create convert button
        self.convert_btn = tk.Button(self.output_frame, text='Convert', command=self.convert)
        ...
        self.convert_btn.grid(row=3, column=0, padx=5, pady=5, sticky='w')
    ...
    def convert(self):
        try:
            ...
            input_value = int(self.input_value.get())
            input_unit = self.input_unit.get()
            output_unit = self.output_unit.get()

            # converts units
            result = self.unit_converter.convert(input_value, input_unit, output_unit)
